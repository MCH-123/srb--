  ...
  methods: {
    ...
    getUserInfo() {
      this.$axios
        ...
        .then((result) => {
          this.borrowerStatus = result.data.borrowerStatus;
    ...
    initSelected() {
      ...
      this.$axios
        ...
        .then((result) => {
          this.educationList = result.data.dictList;
        });
      //行业列表
      this.$axios
        .$get("/api/core/dict/findByDictCode/industry")
        .then((result) => {
          this.industryList = result.data.dictList;
        });
      //收入列表
      this.$axios
        .$get("/api/core/dict/findByDictCode/income")
        .then((result) => {
          this.incomeList = result.data.dictList;
        });
      //还款来源列表
      this.$axios
        .$get("/api/core/dict/findByDictCode/returnSource")
        .then((result) => {
          this.returnSourceList = result.data.dictList;
        });
      //联系人关系列表
      this.$axios
        .$get("/api/core/dict/findByDictCode/relation")
        .then((result) => {
          this.contactsRelationList = result.data.dictList;
        });
    },
